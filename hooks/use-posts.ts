...
export function usePosts() {
    ...
    const fetchPage = async (targetPage: number, force = false) => {
      ...
      `,
        ...
        const newPosts =
            targetPage === 0 ? pagePosts : [...posts, ...pagePosts];
        postsCache = newPosts;
        lastFetchTime = now;
        setPosts(newPosts);
        setPage(targetPage);
            ...
            setPosts((prev) => {
                ...
                const updatedPosts = prev.map((p) => {
                    ...
                    if (
                        ...
                        p.user_has_liked === newUserHasLiked
                    ) {
                        console.log(
                            "Feed: no changes for post",
                            p.id,
                ...
                if (!hasChanges) {
                    console.log(
                        "Feed: no changes in any post - keeping same array reference"
                    );
                    return prev;
                }
                ...
                console.log(
                    "Feed: updated likes cache, posts count:",
                    updatedPosts.length
                );
                return updatedPosts;
            });
    ...
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (!document.hidden && user) {
                console.log(
                    "Feed: tab became visible, force refreshing all data"
                );
